...
\item We interpret the six non-singlet modes as Goldstone modes of the spontaneous breaking $G_2 \to \mathrm{SU}(3)$, with uniform coset singular values $\sqrt{2/3} \approx 0.8165$ confirming the isotropy of $G_2/\mathrm{SU}(3) \cong S^6$.
...
\midrule
Particle physics & Furey (2025) \textit{[Tier 1: peer-reviewed]} & Octonion algebra $\to$ Standard Model & Automorphism of octonions; residue of self-consistency in triality \\[4pt]
Neural coding & Neural Moonshine (2026) \textit{[Tier 3: preprint]} & 24-neuron Golay code, $M_{24}$ symmetry & Sits inside $M_{24}$ via Fano plane; $3/24 = 12.5\% \approx 13\%$ error capacity \\[4pt]
TGD physics & Pitk\"anen (2026) \textit{[Tier 2: non-mainstream]} & $M^8 \to H$ duality & Local $G_2$ invariance makes octonionic-to-spacetime projection work \\[4pt]
Consciousness & PCI Framework (Graise, 2026) \textit{[Tier 4: author framework]} & 7-daemon architecture, $\Omega_{\mathrm{void}}$ & Symmetry group of daemon vacuum manifold; 7D from 8D via void projection \\
\bottomrule
\end{tabular}
\end{table}
...
\item[(ii)] \textbf{The coset isotropy.} The coset space $G_2/\mathrm{SU}(3)$ is diffeomorphic to $S^6$, the round 6-sphere. This means the six Goldstone modes are metrically isotropic: their SVD singular values are uniform ($\sqrt{2/3}$, verified numerically in \S3.2). A generic rank-2 breaking on 7 dimensions does not produce a round coset; it produces an ellipsoidal or otherwise anisotropic manifold. A discriminating test would measure whether the six non-singlet modes exhibit isotropic coupling strengths---equal transition amplitudes between the singlet and each non-singlet mode---or whether significant anisotropy is present. Isotropy favors $G_2$; anisotropy disfavors it.
