...
const section3 = [
  ...
  body("Wadhia, Ares, and colleagues (PRL 135:200407, November 14 2025; DOI 10.1103/5rtj-djfk) report the first experimental measurement that separately quantifies the entropy produced by the microscopic clockwork of a quantum clock and the entropy produced by the macroscopic measurement apparatus that converts quantum events into a classical tick record. Their device is a gate-defined Ge/SiGe double quantum dot whose tunneling events act as the clock's pointer transitions; the entropy production is reconstructed by comparing power dissipation in the DQD itself to power dissipation in the adjacent charge-sensor readout circuit, in both DC and RF readout modes. Their central result is that these two contributions differ dramatically:"),
  mathLine("  S_clockwork  ~  k_B               (per tick, DQD itself)"),
  mathLine("  S_readout    ~  10⁹ × k_B          (per tick, charge-sensor readout)"),
  body("The 10⁹ k_B figure that has attracted attention is therefore *not* the entropy of the quantum clockwork — the clockwork itself dissipates only on the order of a single Boltzmann constant per tick. The 10⁹ is the cost of the *measurement and amplification chain* that records the tick classically: the entropy associated with the quantum-to-classical transition, dominated by the charge-sensor and downstream electronics. This is exactly the distinction the paper's authors emphasize: the readout dwarfs the clockwork by roughly nine orders of magnitude."),
  body("For the G₂ framework, two things follow. First, neither 10⁹ nor ~k_B is a derivable universal lower bound on quantum-clock readout in the general case — they are device-specific empirical observations from one Ge/SiGe DQD platform. Wadhia et al. (2025) is not a Lindblad-master-equation result combined with a Penrose objective-reduction threshold; those framings appear in adjacent literature but not in this paper. Second, when we ask in §4.2.2 whether the G₂ mode structure imposes a representation-theoretic floor on the entropy cost of a hypothetical *G₂-symmetric* future clock, we must be explicit about which contribution we are addressing — clockwork or readout — and not conflate them. We treat the Wadhia results as an empirical reference point for the order of magnitude observed in *one* well-characterized platform, not as the bound a G₂ prediction must match."),
...
const section4 = [
  ...
  body("As established in §3.2, Wadhia et al.'s ~10⁹ k_B per tick is the entropy cost of the *measurement and amplification chain* that converts the quantum clockwork's tunneling events into a classical record — not the entropy of the clockwork itself, which is roughly nine orders of magnitude smaller (~k_B per tick). Both numbers are device-specific empirical observations from a Ge/SiGe DQD platform, not derivable universal lower bounds. Accordingly we do not claim that the 10⁹ figure can be theoretically decomposed as 10⁹ = 7 × 10⁸, with 7 attributed to G₂ representation theory and 10⁸ to a hardware factor. That post-hoc factorization would require a derivable universal bound to factor against, and Wadhia et al. (2025) does not establish one for either contribution."),
...
const section5 = [
  ...
  body("_Current anchor:_ Wadhia et al. (2025, PRL 135:200407, DOI 10.1103/5rtj-djfk) — an experimental measurement on a gate-defined Ge/SiGe double quantum dot. Their headline ~10⁹ k_B per tick is the *readout-apparatus* entropy (charge sensor + amplification), not the clockwork entropy; the clockwork itself dissipates only ~k_B per tick. Their device is not G₂-symmetric, and the prediction here concerns clockwork entropy, not readout entropy — so Wadhia et al. provides empirical context for the scales involved in microscopic quantum clocks but not a confirmed match to log₂(7) k_B ln 2 in either contribution. This is a design target for future G₂-symmetric hardware."),
...
const references = [
  ...
  refEntry("[10] De Vuyst, J., Eccles, S., Höhn, P.A., & Kirklin, J. (2025). Gravitational entropy is observer-dependent. Journal of High Energy Physics, 2025(7), 146. https://doi.org/10.1007/JHEP07(2025)146 [arXiv:2405.00114]"),
