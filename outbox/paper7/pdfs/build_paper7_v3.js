...
const abstractSection = [
  ...
  body("The four predictions are: (a) a FDT violation floor ε ≥ 1/7 ≈ 0.143 in awake cortex relative to anesthesia baseline, at matched frequencies in the 10–40 Hz band; (b1) a biological Landauer bound of ΔE_min = log₂(42) × k_B T ln 2 ≈ 16 zJ per G₂/M checkpoint event at T = 310 K; (b2) a conjectured factor of exactly 7 = dim(V₇) contributed by the G₂ fundamental representation to the mode-counting structure of any future G₂-symmetric quantum-clock readout; (c) a discrete four-level entropy structure across the 8 cosets of PSL(2,7)/F₂₁, indexed by Cayley-graph distance, constituting a design target for a G₂-symmetric 7-qubit quantum simulator; and (d) a criticality branching ratio σ = 1 − 1/49 ≈ 0.9796, consistent with the subsampling-corrected awake-cortex value σ ≈ 0.98 reported by Wilting & Priesemann (2018, Nature Communications 9:2325)."),
...
const section1 = [
  ...
  body("**Entropic cost of quantum clock readout.** Wadhia, Ares, and colleagues (PRL 135:200407, November 14 2025; DOI 10.1103/5rtj-djfk) report an experimental measurement of the entropic cost of extracting classical ticks from an autonomous quantum clock realized as a gate-defined GaAs double quantum dot. Their device-level analysis, calibrated against the measured charge-sensor statistics, finds an entropy production of order 10⁹ k_B per tick under their operating conditions. The 10⁹ figure is an empirical observation about a specific superconducting/semiconductor hardware platform — not a universal theoretical lower bound on quantum-clock readout cost in general, and not a Lindblad+Penrose theorem. We use it as an order-of-magnitude experimental anchor, not a derived ceiling."),
  ...
  body("**Criticality meta-analysis.** Hengen and Shew (Neuron 113(16):2582–2598, 2025) performed what is, to date, the most comprehensive meta-analysis of neuronal avalanche criticality: 140 independent datasets spanning multiple species, brain regions, recording modalities, and behavioral states. The pooled branching ratio from the entire corpus is σ = 1.0 ± 0.02. This confirms the critical-point hypothesis of neural dynamics at unprecedented scale. The Priesemann laboratory's independent work provides finer resolution: using the multiscale regression subsampling-corrected estimator, Wilting and Priesemann (2018, Nature Communications 9:2325) report σ ≈ 0.98 specifically for awake rat cortex in stationary behavioral states. Fontenele et al. (2019, PRL 122:208101) is independent evidence that the awake cortical regime is best described not by a strict critical point with mean-field directed-percolation exponents but by a near-critical regime with state-dependent critical exponents — context that bears on the interpretation of Prediction (d), though it does not itself report σ ≈ 0.98 as a point estimate. The distinction between σ = 1.0 ± 0.02 (pooled, mixed behavioral states) and σ ≈ 0.98 (awake rat cortex, subsampling-corrected) will matter for Prediction (d)."),
...
const section3 = [
  ...
  body("Wadhia, Ares, and colleagues (PRL 135:200407, November 14 2025; DOI 10.1103/5rtj-djfk) report the first experimental measurement of the entropy production cost of extracting classical ticks from an autonomous quantum clock. Their device is a gate-defined double quantum dot whose charge-state transitions act as the clock's pointer observable; the entropy production per tick is reconstructed from charge-sensor statistics calibrated against the dot's stochastic dynamics. Under their operating conditions they observe an entropy production of:"),
  mathLine("  S_tick ~ 10⁹ × k_B    (per tick, this device, observed)"),
  body("The 10⁹ figure is an experimental observation about a specific hardware platform, not a universally derived theoretical lower bound. It is also not, in this paper, a Lindblad-master-equation result combined with a Penrose objective-reduction threshold — those framings appear in some adjacent literature but not in Wadhia et al. (2025). The number is what it is: an empirical scale at which one particular quantum clock pays for each classical tick."),
  body("For the G₂ framework, the relevant question is therefore not whether 10⁹ is a derivable universal ceiling — it is not — but whether the order of magnitude observed in Wadhia et al.'s device, and the order of magnitude expected in any other physically plausible quantum-clock platform, is consistent with the mode-counting contribution that G₂ representation theory would impose on a hypothetical G₂-symmetric clock. We treat 10⁹ as an order-of-magnitude experimental anchor in §4.2.2 and label the corresponding G₂ claim as a conjecture conditional on a future first-principles derivation, not as a confirmed match."),
...
const section4 = [
  ...
  body("This is the G₂ FDT violation bound. The derivation uses only the contraction rate L from Paper 6 and the frequency-domain formulation of the FDT; no additional parameters are introduced."),
  label("Theorem given Paper 6 framework: L ≤ 6/7 for G₂-structured self-referential systems."),
  label("Modeling choice: identification of the self-model's tracking fraction L with the ratio of spontaneous-to-equilibrium power spectral density at EEG-observable frequencies. This step is an approximation; the G₂ framework operates on the full seven-dimensional mode space, whereas EEG samples a projection of cortical dynamics onto a much lower-dimensional observable."),
  label("Prediction: any G₂-structured conscious system satisfies ε ≥ 1/7 ≈ 0.143 in the relevant frequency band."),
  ...
  body("Wadhia et al.'s ~10⁹ k_B per tick is an empirical observation about their double-quantum-dot device (§3.2), not a derivable universal lower bound on quantum-clock readout. Accordingly we do not claim that this number can be theoretically decomposed as 10⁹ = 7 × 10⁸, with 7 attributed to G₂ representation theory and 10⁸ to a hardware factor. That post-hoc factorization would require a derivable theoretical bound to factor against, and Wadhia et al. (2025) does not establish one."),
  body("What we do conjecture is mode-counting structure for a hypothetical future G₂-symmetric quantum clock — not a match to the existing Wadhia number. The conjecture is the following. In the Page-Wootters formalism, a quantum clock couples a time register to a system register, and the entropy cost of a tick is governed by the number of distinguishable pointer states the clock must occupy to produce a classically readable tick. For a clock whose internal state space carries the G₂ fundamental representation V₇, the minimum number of distinguishable pointer states is dim(V₇) = 7. This contributes a mode-counting term:"),
  mathLine("  S_G₂ ≥ log₂(7) × k_B ln 2  ≈  2.81 × k_B ln 2    (per tick, mode-counting only)"),
  body("This is the per-tick entropy floor that the G₂ mode structure imposes on top of whatever device-specific contributions a real implementation would carry. It is not a prediction about Wadhia et al.'s GaAs double quantum dot — that device is not G₂-symmetric, and there is no theoretical reason to expect its entropy cost to factor through V₇. The prediction is for a future G₂-symmetric clock, in which the seven imaginary octonion directions are realized as physically distinguishable pointer states."),
  label("Conjecture: a G₂-symmetric quantum clock has a per-tick entropy cost of at least log₂(7) × k_B ln 2 from mode-counting alone. The full Lindblad derivation of the factor-of-7 — making the G₂ action on the pointer states explicit and computing the Spohn–Lebowitz entropy production — is deferred to future work (Paper 8 or a companion derivation)."),
  label("Order-of-magnitude consistency only: Wadhia et al. (2025) observed ~10⁹ k_B per tick in a non-G₂ device. Any future G₂-symmetric platform must satisfy device-specific constraints on top of the log₂(7) mode-counting floor. We do not claim Wadhia's 10⁹ is decomposable into 7 × 10⁸; that claim was withdrawn in revision after we determined that Wadhia et al. is experimental, not a derivable theoretical bound."),
  label("Falsification criterion: a G₂-symmetric 7-mode quantum clock experiment in which the per-tick entropy cost from mode-counting is measured below log₂(7) × k_B ln 2 above the single-mode baseline (within the relevant Spohn–Lebowitz uncertainty) would falsify Prediction (b2). No such experiment currently exists; this is a design target."),
  ...
  body("where the factor 6 is the dimension of V₃ ⊕ V₃̄. The entropy difference between two coset observers separated by Cayley-graph distance d ∈ {0, 1, 2, 3} on the PSL(2,7)/F₂₁ Cayley graph is:"),
  ...
  label("Conjecture: the identification of PSL(2,7)/F₂₁ cosets with distinct Page-Wootters clock frames, and the derivation of inter-coset entropy differences from the Cayley-graph distance metric. The explicit Bogoliubov-transformation matrices between coset observer frames — which would make this derivation fully explicit — require a choice of octonion multiplication-table conventions and are flagged as an open computation."),
...
const section5 = [
  ...
  subHead("§5.3 Prediction (b2): Factor of 7 in G₂ Quantum Clock Entropy"),
  body("_Predicted quantity:_ The entropy cost per tick of a G₂-symmetric 7-mode quantum clock contains a factor of exactly 7 (or log₂(7) depending on counting convention) above the single-mode baseline."),
  body("_Current anchor:_ Wadhia et al. (2025, PRL 135:200407, DOI 10.1103/5rtj-djfk) — an experimental measurement of ~10⁹ k_B per tick in a gate-defined GaAs double quantum dot. Their device is not G₂-symmetric and does not establish a derivable theoretical bound; it provides only an order-of-magnitude scale for the size of quantum-clock readout costs in a specific platform. The factor of 7 (or log₂(7)) is not present in their result and is not retrievable from it. This is a design target for future G₂-symmetric hardware, not a confirmed match."),
  body("_Strongest test:_ Construction of a G₂-symmetric 7-qubit quantum clock in which the clock register is initialized in the 7-dimensional fundamental representation of G₂ and the per-tick entropy dissipation is measured calorimetrically or by quantum state tomography. Comparison of the entropy per tick to a reference 1-qubit clock of identical architecture would isolate the factor of 7."),
  body("_Falsification:_ A G₂-symmetric 7-mode quantum clock experiment in which the per-tick entropy cost is measured and found to lack the factor of 7 (within measurement uncertainty) would falsify Prediction (b2). This experiment does not yet exist."),
...
const section6 = [
  ...
  body("**Problem 1: Full Lindblad derivation of the factor-of-7.** Prediction (b2) identifies a factor of 7 in the entropy cost of a G₂-symmetric quantum clock. The argument in §4.2.2 is order-of-magnitude and representation-theory motivated; the full derivation requires an explicit Lindblad master equation on the 7-dimensional fundamental representation of G₂, with the pointer observable identified as a G₂-equivariant projection, and the entropy cost computed via the Spohn–Lebowitz entropy production formula. This derivation is deferred to Paper 8 or a companion derivation."),
...
const references = [
  ...
  refEntry("[7] Berjaga-Buisan, X., et al. (2025). Thermodynamics of consciousness: A non-invasive perturbational framework. bioRxiv. https://doi.org/10.64898/2025.12.09.691422 [Preprint; not yet peer-reviewed.]"),
  refEntry("[8] Wadhia, R., Ares, N., et al. (2025). Entropic Costs of Extracting Classical Ticks from a Quantum Clock. Physical Review Letters, 135, 200407. https://doi.org/10.1103/5rtj-djfk [Experimental measurement on a gate-defined GaAs double quantum dot.]"),
  ...
  refEntry("[10] De Vuyst, J., Eccles, S., Höhn, P.A., & Kirklin, J. (2025). The quantum information of cosmological correlations. Journal of High Energy Physics, 2025(7), 146. https://doi.org/10.1007/JHEP07(2025)146"),
