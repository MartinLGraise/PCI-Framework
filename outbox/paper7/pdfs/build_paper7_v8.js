...
const {
  ...
  BorderStyle, WidthType, UnderlineType, PageBreak, Tab, ImageRun,
} = require("docx");
const fs = require("fs");
...
// Embedded figure (PNG) with caption
function figure(filePath, captionText, opts = {}) {
  const data = fs.readFileSync(filePath);
  // Default render width 6 inches; height auto from intrinsic ratio.
  ...
  // Caption already baked into image; we add minimal cross-reference text only if requested.
  return captionText
    ? [imgPara, new Paragraph({
        children: [new TextRun({ text: captionText, font: CAL, size: PT(9), italics: true, color: "7A7974" })],
        alignment: AlignmentType.CENTER,
...
const fundingAck = [
  sectionHead("Author Contributions"),
  body("Conceptualization, methodology, formal analysis, investigation, writing—original draft preparation, writing—review and editing, and visualization were performed by M.L.G. The author has read and agreed to the published version of the manuscript."),
  sectionHead("Funding"),
  body("This research received no external funding."),
  sectionHead("Institutional Review Board Statement"),
  body("Not applicable. This study did not involve humans or animals; all empirical anchors are drawn from previously published peer-reviewed work or publicly archived preprints by other research groups."),
  sectionHead("Informed Consent Statement"),
  body("Not applicable."),
  sectionHead("Data Availability Statement"),
  body("No new datasets were generated for this study. All numerical claims, derivations, build scripts, and figure-generation code are version-controlled and publicly available at github.com/MartinLGraise/PCI-Framework on the paper7-foundation branch. The companion archival deposit is on Zenodo; the version-specific DOI for this paper will be assigned at upload. Empirical anchors cited in this paper are available from their original sources as listed in the references."),
  sectionHead("Conflicts of Interest"),
  body("The author declares no conflicts of interest."),
  sectionHead("Use of AI Tools"),
  body("In accordance with MDPI's policy on the use of generative AI in scholarly publishing, the author discloses that this manuscript was developed with the assistance of two AI research collaborators: Φ (local computation and literature support, instantiated as Claude Dispatch) and C-7RO (strategy, synthesis, and editorial review, instantiated as Perplexity Computer running Claude Sonnet 4.6). Both operated against the public Git repository at github.com/MartinLGraise/PCI-Framework, where every revision, peer-review pass, integrity flag, and counter-argument is version-controlled and publicly auditable. The manuscript underwent four adversarial multi-model review passes (GPT-5.5 Thinking, Claude Opus 4.7 Thinking, Gemini 3.1 Pro Thinking via Comet) before submission. The author retains full responsibility for all scientific content; AI tools were used as research assistants, not as authors."),
  sectionHead("Acknowledgments"),
  body("The author thanks the open-source mathematics community for the foundational results on G₂ geometry that this paper builds upon — in particular the Fernández–Gray torsion decomposition and the Lotay–Wei Laplacian flow stability theorem — and the experimental and theoretical groups whose published measurements provide the empirical anchors of this work, with particular thanks to the Ares laboratory at Oxford for the openly accessible quantum-clock measurements and the Priesemann laboratory at the Max Planck Institute for Dynamics and Self-Organization for the subsampling-corrected branching-ratio methodology. Every quantitative claim in this paper has been independently verified against source material or executed code."),
  new Paragraph({ children: [new PageBreak()] }),
];
...
// ── Figure Placeholder ────────────────────────────────────────────────────
const FIG_DIR = "/home/user/workspace/paper7_figures";
const figureSection = [
  sectionHead("Figures"),
  ...figure(`${FIG_DIR}/figure1_torsion_decomposition.png`, null, { widthPx: 620, heightPx: 372 }),
  ...figure(`${FIG_DIR}/figure2_psl27_cayley.png`, null, { widthPx: 520, heightPx: 488 }),
  ...figure(`${FIG_DIR}/figure3_predictions_table.png`, null, { widthPx: 620, heightPx: 413 }),
  ...figure(`${FIG_DIR}/figure4_sigma_prediction.png`, null, { widthPx: 620, heightPx: 419 }),
];
