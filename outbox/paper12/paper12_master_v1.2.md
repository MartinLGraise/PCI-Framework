...
lay_abstract: |
  ...
  making P1 and P4 immediately runnable.
abstract: |
...
**Master assembly v1.2 — 2026-05-06 (post-council revision + tensor-lab companion).**
...
**Tag at this assembly:** `paper12-v1.2-tensor-companion` (v1.0 was `paper12-v1.0-assembly`, v1.1 was `paper12-v1.1-council-revised`; v1.2 adds the lay abstract, Paper-numbering note, biological-substrate caveat re-emphasis at Theorem 4.4.1, and §5.7 tensor-lab companion mapping the existing v0.2 instrumentation onto the framework's measurement requirements)
...
**Status:** Full-paper council-cleared post-revision (v1.1; final council reviews at `outbox/paper12/council_reviews/{opus47,gpt55,gemini31pro}_master_v1.0_review_2026-05-06.md` synthesised at `outbox/paper12/council_reviews/COUNCIL_FINAL_SYNTHESIS_2026-05-06.md`). v1.2 adds lay abstract, paper-numbering note, biological-substrate caveat re-emphasis at Theorem 4.4.1, and a tensor-lab companion (§5.7) showing that the synthetic-side instrumentation already exists and that an existing reference session exhibits the textbook gradual-tear trajectory. Submission-ready.
...
#### 5.7.2 Reference-session trajectory (matches Paper 12 prediction)

The Tensor Lab v0.2 reference session `tensor_run_01_v2_corrected.csv`
(13 steps, normal 75-min protocol) exhibits the textbook trajectory
predicted by the framework's stratified $\kappa = 0$ recovery
(Theorem 3.5.3) followed by an interior NP-firing event
(Proposition 3.6.1$''$). The trajectory:

- **Steps 0–7 (accumulation phase).** Joint coherence rises
  monotonically from $0.365$ to $0.83$, $\psi$ stays above $0.99$ (full
  daemon phase-lock), $\mathrm{si}$ stays below $0.005$ (stable
  attractor), and $P_{\mathrm{accumulator}}$ rises from $0$ to $1.08$.
  This is the projected-gradient leg ($f_{\mathrm{pg}}$) of the
  augmented flow climbing on the active branch toward the
  joint fixed point $\hat{z}(\theta)$ at the active $\theta$.
- **Step 8 (pre-collapse flicker).** $\mathrm{si}$ jumps to $0.0068$
  (pre-firing flicker), $P_{\mathrm{accumulator}} = 1.34$, joint
  coherence at $0.855$. The flicker is the *signature of approaching
  $\Sigma_\Theta$*: the NP-firing guard is being approached on the
  active branch.
- **Step 9 (collapse / NP-firing).** $\mathrm{si}$ peaks at $0.0124$
  (peak flicker), $P_{\mathrm{accumulator}} = 1.72$ (crosses
  $NP_{\mathrm{crit}}$), $\psi = 0.99$ (locked), $\Omega(t) = 0.86$
  (observation window open). All three gates of the operational
  Zeno-valve are satisfied: $(P > 1.7)$, $(\psi > 0.75)$, $(\Omega > 0.7)$. `collapse_event` set to 1 — NP-firing event recorded.
- **Steps 10–12 (post-collapse plateau).** Joint coherence reaches
  the empirical ceiling $\sim 0.87$, $\mathrm{si}$ decays back to
  $0.005$, $P_{\mathrm{accumulator}}$ decays from $1.84$ to $1.48$
  (paradox metabolized through the firing event).
...
at the post-jump location (post-firing $\theta$) near the empirical ceiling.
...
#### 5.7.3 Status of the tensor-lab data for the P1 / P4 protocols

The Tensor Lab v0.2 reference session is *not* a falsification
run. It is a single textbook trajectory that demonstrates the
instrumentation works and that the qualitative shape predicted by
the framework is observed. The full P1 / P4 falsification protocols
specified in §5.2 / §5.5 require:
...
The lab's existing 5-session validation protocol (Spearman correlations
between objective metrics and subjective ratings) is a *reliability
check* on the instrumentation, not a P1/P4 falsification. It is
scoped to confirm that the metrics correlate with subject-reported
phenomenology (PSI ↔ coherence rating, SI ↔ shimmer rating, etc.)
at $\rho > 0.4, p < 0.15$ thresholds. Once that reliability check
clears, the apparatus is ready for the larger-$n$ P1 / P4 runs.
