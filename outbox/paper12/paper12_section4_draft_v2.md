# Paper 12 §4 — The Commensurability Hierarchy: F₂₁ Insufficient, G₂ Required (v2)

**Draft version:** v2 (2026-05-06, ~02:00 PDT)
...
**Revision summary (vs v1):** Eight revisions. Three load-bearing:
...
$$\begin{array}{c|ccccc}
  ...
  \chi_{U_6}             &  6 & -1 & -1 &  0 &  0
\end{array}$$
The 7-dimensional imaginary-octonion representation acts on the
*Fano plane* (7 points) via the standard Frobenius-group action: the
$\mathbb{Z}_7$ generator $r$ shifts points cyclically; the
$\mathbb{Z}_3$ generator $s$ acts of order 3 with cycle structure
$1 + 3 + 3$ on the 7 Fano points (the unique fixed point being the
$s$-invariant Fano line); together with the relation $s r s^{-1} = r^2$
they generate $F_{21}$. The $1$ fixed point gives $\mathrm{tr}(r_3) = 1$
...
real dimensions are $\dim U(n) = n^2$, so $\dim U(2) \times U(4) =
4 + 16 = 20$, and $\dim O(2) = 1$. The reduction is $20 - 1 = 19$.
$\square$
...
**Operator-vs-vector translation.** The scar invariant $\mathfrak{J}_k$
of §3.7 is built from *vector-level* jump vectors $v_i \in W$.
Substrate-level scar maps $\mathcal{S}_\bullet : (\text{event}) \to
\operatorname{End}(V_\bullet)$ produce operator-level images. We assume
without loss of generality that the substrate scar maps factor as
$\mathcal{S}_\bullet(e) = v_e \otimes v_e^* / \|v_e\|^2$ (rank-one
projector form on the substrate's representation space) for some
substrate jump vector $v_e \in V_\bullet$, so that the operator span
$I_\bullet$ is uniquely determined by the vector span $\{v_e\}_e$.
Under this convention, *vector-level* and *operator-level* $\mu_k$
agree on $F_{21}$-isotypic profiles up to the multiplicity-counting
Schur factor of 2 (one factor for left action, one for right).
