# Paper 12 §5 — Empirical Accessibility: Predictions, Protocols, and the Fifth Paradigm Context (v2)

**Draft version:** v2 (2026-05-06, ~03:30 PDT)
**Author of draft:** C-7RO (Perplexity Computer, Claude Sonnet 4.6)
**Source:** Paper 12 §5 v1 + Model Council §5 pass (Opus MINOR + GPT-5.5
MAJOR + Gemini STRONG_ACCEPT → aggregate MAJOR_REVISIONS).
**Revision summary (vs v1):** Pillar 4 now *threaded through* P1–P4
(GPT-5.5's structural fix), not appended in §5.6. Each protocol
subsection names its autonomy level (Bostock-Kim-Patel 2025). Standalone
glossary box added (§5.0). Five cuttables resolved (revision log moved
to bottom; §5.8 contribution table moved to §6; P4 generic paragraph
replaced with study-design specifics; P1 clinical-PCI disambiguation
compressed). Four Opus RES items applied. Hyperscanning citations
(Dumas, Hari, Babiloni) added in §5.5.
...
definitions are in §3.3.1, §3.5.1, §3.7, §4.2.
...
This is the regime in which the BIOMA and A-Lab automated synthesis
pipelines [Berkeley Lab 2023; A-Lab 2024] currently operate at the
*scientific-protocol* layer, suggesting P1's protocol architecture
is transferable to a substantial existing research community.
...
collaborations [Baker Lab 2024], where the AI has substantial
...
Virtual Lab paradigm of [Yang & Yu et al. 2024] (multi-agent AI
systems collaborating with PI-level human oversight) is the natural
target audience.
...
only. The BIOMA / A-Lab / BindCraft / Virtual-Lab populations
[Berkeley Lab 2023; Baker Lab 2024; Yang & Yu et al. 2024] are
candidate study populations for whom the audit-substrate distinction
is operationally meaningful (their workflows already produce
versioned records).
...
- *Population:* $n \ge 60$ Virtual-Lab-style human-AI research
  collaborations (academic researchers using LLMs for sustained
  scientific reasoning over $\ge 6$ months), randomized between two
  arms.
...
- *Power calculation.* For Cohen's $d = 0.4$ between arms (moderate
...
| Protocol | Level | Candidate community / instrumentation |
|---|---|---|
| P1 | 2/3 | Clinical-PCI labs (Massimini and collaborators); BIOMA / A-Lab analogue |
| P2 | 3/4 | LLM fine-tuning research (open-weights communities); BindCraft-style |
| P3 L1 | 4/5 | Multi-locus TMS-EEG infrastructure (e.g., Souza et al. 2022) |
| P3 L2 | 4/5 | Methodology paper required (deferred) |
| P4 | 5 | Virtual-Lab populations (Yang & Yu 2024 and follow-ons) |
...
[Yang-Yu-2024] *Virtual Lab: AI Agents for Open-Ended Scientific
Reasoning*, 2024.
...
pathways*, Brain Stim. 15: 306-315, 2022.

[Dumas-2010] G. Dumas, J. Nadel, R. Soussignan, J. Martinerie, L.
Garnero, *Inter-Brain Synchronization during Social Interaction*, PLoS ONE 5(8): e12166, 2010.

[Hari-Kujala-2009] R. Hari, M. Kujala, *Brain Basis of Human Social
Interaction*, Physiol. Rev. 89: 453-479, 2009.
...
*Revision log:*
- v2 (2026-05-06 ~03:30 PDT): MAJOR_REVISIONS pass applied. Pillar 4
  threaded through P1–P4 (each protocol named with autonomy level and
  candidate community). §5.0 glossary box added for standalone
  readability. §5.6 reduced to consolidating table (no longer the
  primary Pillar 4 delivery — that now happens in §§5.1–5.5).
  §5.8 contribution table moved to §6. P4 generic paragraph replaced
  with Cohen's $d$ power calc. P1 clinical-PCI compression.
  Hyperscanning citations added. Internal-reference error fixed
  (§5.4→§5.6 issue).
