...
by Theorem 9.2 (for scalar $R$) and Lemma 5.2.1 below (for general
orthogonal $R$).*
...
Part (d) follows from Theorem 9.2 (when $R_A, R_B$ are scalar multiples
of the identity, i.e., fully G₂-equivariant) and from Lemma 5.2.1 below
(when $R_A, R_B$ are arbitrary orthogonal matrices, as in the Φ
verification setup). $\square$
...
*(a) **Numerical existence of an improvement region.** Numerically
(Appendix V.3 Task 5.4, 100 × 20 grid in $(\varphi, \theta)$-space at the
seeded geometry), there exists a non-empty subset
$\widehat{\mathcal{R}} \subset (\varphi, \theta) \in (0, \pi) \times (0, \pi/2)$
of positive Lebesgue measure on which*
$\mathcal{C}_{\mathrm{avg}}(\theta) > \mathcal{C}_{\mathrm{avg}}^{\mathrm{dec}}$;
*the corresponding region for $\mathcal{C}_{\min}$ is bounded above by
$\widehat{\mathcal{R}}$ and is the subject of an extended verification
run reported in Appendix V.3.f.*
...
**Proof.** Part (a): direct from the heatmap of Appendix V.3.f, which
reports the explicit fraction of the grid on which improvement holds.
Part (b): the $\mathcal{C}_1$ trajectory of Appendix V.3 Task 5.6 falls
from $0.7655$ at $\theta = 0$ to $0.5955$ at $\theta = \pi/2$ along the
sampled grid; combined with $\mathcal{C}_{\min} \le \mathcal{C}_1$ this
gives the stated inequality. Part (c): the optimal angles are read off
...
with the improvement region extending to $\varphi$ near $180°$ at small
$\theta$. Under $\mathcal{C}_{\min}$, the cells where
$\mathcal{C}_2 > \mathcal{C}_1$ along with $\mathcal{C}_1 < \mathcal{C}_1^{\mathrm{dec}}$
...
geometry). The figure in §6 visualizes the $\mathcal{C}_{\mathrm{avg}}$
heatmap with the $\mathcal{C}_{\min}$-corrected region overlaid.
...
![Paper 9, Fig. 1: Conditional improvement region for the seeded G₂-structured affine dyad. Left: the heatmap of $\Delta\mathcal{C}_{\mathrm{avg}}(\varphi, \theta)$ over the bias-alignment angle $\varphi \in [0, 180°]$ and coupling angle $\theta \in [0, 90°]$. Red cells indicate joint-coherence gain, blue cells indicate degradation. Right: the binary gain region ($\Delta\mathcal{C}_{\mathrm{avg}} > 0$) shaded; the dashed line near $\varphi = 170°$ marks the opposed-bias boundary that Proposition 9.5(b) excludes from the gain region under the seeded geometry. About 25% of the parameter space exhibits positive conditional gain; the remaining ~75% exhibits degradation. Generated from Appendix V.3 Task 5.4 raw CSV; reproducible from `paper9_verification_v3.py` with `seed = 20260504`.](figures/paper9_fig1_heatmap.png)
...
*End of Paper 9 v1.3 master draft, consolidated 2026-05-04 19:20 PDT.*
