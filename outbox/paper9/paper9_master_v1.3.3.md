...
**Date:** 2026-05-05
**Manuscript version:** v1.3.3 (errata-only update on v1.3.2)
**DOI (v1.3.2 of record):** [10.5281/zenodo.20034821](https://doi.org/10.5281/zenodo.20034821)
**v1.3.3 supersedes v1.3.2** with a single addendum (Remark 5.6.1 below) recording a grid-resolution correction discovered during Paper 12 §3 verification on 2026-05-06; substantive results unchanged.
...
is $\approx 51°$ in the symmetric-rate case **on the 20-point
$\theta$-grid of Appendix V.3 Task 5.5** (see Remark 5.6.1 for the
high-precision reoptimization) and $\approx 75°–80°$ in the
asymmetric-rate case (Appendix V.3 Tasks 5.1, 5.5), establishing that
$\theta^*$ is not generically at $\pi/4$.*

**Remark 5.6.1 (Grid-resolution correction, errata-only addendum, v1.3.3).**
The $\theta^* \approx 51°$ value reported in (c) for the seeded
symmetric-rate geometry is *snapped to the 20-point $\theta$-grid*
used throughout Appendix V.3 (resolution $\Delta\theta = \pi/2/19
\approx 4.74°$). Reoptimization with `scipy.optimize.minimize_scalar`
on $[0, \pi/2]$ at tolerance $10^{-10}$ rad, performed during Paper 12
§3 verification (2026-05-06), yields the corrected high-precision
optimum
$$\theta^*_{\mathrm{seed-}k=0}\Big|_{\varphi = 0,\, r_A = r_B = 0.7}
  \;=\; 0°$$
for seeds $(R_A, R_B) =$ (20260504, 20260505), with
$\mathcal{C}_{\min}(\theta^*) = 0.7401$ (the decoupled value
$\mathcal{C}_{\min}^{\mathrm{dec}}$). The high-precision optimum is
on the *boundary* $\theta = 0$ of the closed interval $[0, \pi/2]$,
not in the interior near $51°$. The grid-snap occurred because
Proposition 9.5's $\mathcal{C}_{\min}$ landscape over $[0, \pi/2]$
is nearly flat near $\theta = 0$ for this seed, and the 20-point
grid's nearest interior maximum at $\theta_{\mathrm{grid}} \approx 51°$
differs from the true boundary value by only $\sim 10^{-4}$.

This correction does *not* invalidate any substantive Proposition 9.5
result:

- *Part (a)* (existence of a non-empty $\mathcal{C}_{\min}$-improvement
  region for the seeded geometry) is unchanged. The improvement region
  is computed on the full $(\varphi, \theta)$ grid, not at the seeded
  geometry's $\theta^*$ specifically.
- *Part (b)* (degradation under opposed biases for the seeded geometry)
  is unchanged. The $\mathcal{C}_1(\theta)$ trajectory of Appendix V.3
  Task 5.6 reports $\mathcal{C}_1(0) = 0.7655 > \mathcal{C}_1(\theta)$
  for all $\theta \in (0, \pi/2]$ tested, which is consistent with
  $\theta^* = 0$ rather than against it.
- *Part (c)*'s qualitative conclusion ($\theta^*$ is not generically at
  $\pi/4$) is *strengthened*. With the correction, $|\theta^* - 45°| =
  45°$ for the seeded geometry, comfortably exceeding the $5°$
  separation reported in the 50-seed Monte Carlo (§5.6, Appendix V.3.i)
  and joining the boundary-KKT class identified in Paper 12 §3's
  $\kappa = 0$ recovery audit (50 of 50 seeds satisfy
  $|\theta^*_{\mathrm{reopt}} - 45°| > 5°$ under high-precision
  reoptimization).

The asymmetric-rate $\theta^* \approx 75°–80°$ value reported in (c)
for Task 5.1 has not been reoptimized at high precision in v1.3.3 and
remains a grid-snapped value pending Paper 12 §3 verification of the
asymmetric-rate case.

Figure 1's heatmaps and Conjecture 9.5′'s 50-seed Monte Carlo
statistics in (§5.6, Appendix V.3.i) are unaffected by this
correction — the MC reports per-seed $\theta^*$ on the *same* 20-point
grid as the heatmap, so the grid-snap effect is consistent across the
ensemble. The Paper 12 §3 audit (`outbox/paper12/computations/
paper12_q5_kappa0_audit.csv`, run 2026-05-06) reoptimizes all 50 seeds
at high precision and finds:

- **3/50** seeds with $\theta^*_{\mathrm{reopt}}$ in the smooth interior
  (Fermat critical points)
- **25/50** seeds with $\theta^*_{\mathrm{reopt}} = 0°$
  (boundary-left KKT, including seed-$k = 0$)
- **3/50** seeds with $\theta^*_{\mathrm{reopt}} = \pi/2$
  (boundary-right KKT)
- **19/50** seeds with $\theta^*_{\mathrm{reopt}}$ on the
  $\Sigma_{\min} = \{c_A = c_B\}$ locus (Clarke generalized-gradient
  critical points)

The MC's qualitative conclusions (50/50 non-empty improvement regions,
50/50 satisfy $|\theta^* - 45°| > 5°$) survive the correction; the
*nature* of those optima is now classified as predominantly
boundary/stratified rather than smooth-interior. Paper 12 §3 develops
the projected-gradient framework that handles this stratification
canonically.

*This remark is the only substantive change between v1.3.2 and v1.3.3.*
...
*End of Paper 9 v1.3.3 master draft (errata-only update on v1.3.2),
...
**v1.3.3 (2026-05-06)** — Errata-only update on v1.3.2. Added Remark 5.6.1 recording the grid-resolution correction for the seeded symmetric-rate $\theta^*$ (snapped $51° \to$ high-precision $0°$ on the boundary). Substantive Proposition 9.5(a,b) results unchanged; (c)'s qualitative conclusion $\theta^* \ne \pi/4$ is *strengthened*. Companion Paper 12 §3 audit (`paper12_q5_kappa0_audit.{py,csv}`, 2026-05-06) reoptimizes all 50 seeds at high precision and reports the boundary/stratified classification. Correction discovered during Paper 12 §3 pre-drafting verification.
